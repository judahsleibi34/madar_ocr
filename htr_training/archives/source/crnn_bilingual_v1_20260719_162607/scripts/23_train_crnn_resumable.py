...
def restore_rng_state(
    ...
    """
    Restore RNG state safely even when a checkpoint was loaded
    onto CUDA or was created with a different GPU count.
    """
    if not state:
        return
    ...
    cuda_state_values = state.get("cuda")

    if (
        torch.cuda.is_available()
        and cuda_state_values is not None
    ):
        if torch.is_tensor(cuda_state_values):
            cuda_state_values = [
                cuda_state_values
            ]
        ...
        for cuda_state in cuda_state_values:
