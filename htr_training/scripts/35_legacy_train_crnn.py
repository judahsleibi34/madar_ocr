...
def calculate_ctc_loss(
    ...
    input_lengths = model.calculate_input_lengths(image_widths)
...
def main() -> None:
    ...
    optimizer = AdamW(model.parameters(), lr=arguments.learning_rate, weight_decay=1e-4)
    ...
    for epoch in range(1, arguments.epochs + 1):
        ...
        train_loss = train_one_epoch(
            ...
            scaler=scaler, device=device, use_amp=use_amp, gradient_clip=arguments.gradient_clip,
            log_every=arguments.log_every, accumulation_steps=arguments.accumulation_steps,
        )

        validation_results = validate(
            model=model, loader=validation_loader, criterion=criterion,
        ...
        save_checkpoint(
            checkpoint_path=latest_path, model=model, optimizer=optimizer, epoch=epoch,
        ...
        if validation_loss < best_validation_loss:
            ...
            save_checkpoint(
                checkpoint_path=best_path, model=model, optimizer=optimizer, epoch=epoch,
                train_loss=train_loss, validation_loss=validation_loss,
                arguments=arguments, num_classes=num_classes,
            )
